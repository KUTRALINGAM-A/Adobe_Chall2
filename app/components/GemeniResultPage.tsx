...
const [showTextSelectionModal, setShowTextSelectionModal] = useState<boolean>(false);
...
const handleTextSelection = useCallback(() => {
  ...
  setTimeout(() => {
    ...
    if (selection && selection.toString().trim().length > 0) {
      ...
      // Only trigger if selected text is substantial (more than 10 characters)
      if (selectedText.length > 10) {
  console.log('Text selected:', selectedText);
  setSelectedPdfText(selectedText);
  setShowTextSelectionModal(false); // do NOT show modal
  searchWithSelectedText(true); // immediately trigger search
}
...
useEffect(() => {
  ...
  const addSelectionListener = () => {
    // Try multiple selectors for the PDF container
    pdfContainer = document.getElementById('adobe-dc-view') || 
                   document.querySelector('[data-adobe-dc-view]') ||
                   document.querySelector('.adobe-dc-view');
    
    if (pdfContainer) {
      console.log('Adding text selection listeners to PDF container'); // Debug log
      pdfContainer.addEventListener('mouseup', handleTextSelection);
      pdfContainer.addEventListener('touchend', handleTextSelection);
      pdfContainer.addEventListener('selectionchange', handleTextSelection);
  ...
  const removeSelectionListener = () => {
    if (pdfContainer) {
      console.log('Removing text selection listeners'); // Debug log
      pdfContainer.removeEventListener('mouseup', handleTextSelection);
      pdfContainer.removeEventListener('touchend', handleTextSelection);
      pdfContainer.removeEventListener('selectionchange', handleTextSelection);
  ...
  // Add listener when PDF is loaded
  if (viewingPdf) {
    // Multiple attempts with different delays
    setTimeout(addSelectionListener, 1000);
    setTimeout(addSelectionListener, 2000);
    setTimeout(addSelectionListener, 3000);
  }

  return removeSelectionListener;
}, [viewingPdf, handleTextSelection]);
...
const debugSearch = () => {
  if (selectedPdfText) {
    console.log('Debug: Searching with selected text:', selectedPdfText);
    searchWithSelectedText(true);
  } else {
    console.log('Debug: No selected text available');
  }
};
...
const searchWithSelectedText = async (useSelectedText: boolean = true) => {
  const searchQuery = useSelectedText ? selectedPdfText : searchText;
  
  if (!searchQuery.trim() || documents.length === 0) {
    alert('No search text available or documents are empty');
    return;
  }

  // Set the search text in the input
  ...
  try {
    const validDocuments = documents.filter(doc => doc.file || doc.fileData);
    if (validDocuments.length === 0) {
      alert('No valid PDF files found. Please upload some PDFs.');
      setIsSearching(false);
      return;
    ...
    const documentContents = await Promise.all(
      validDocuments.map(async (doc) => {
        ...
        if (doc.file) {
          ...
          // Create a temporary file from fileData
          let arrayBuffer: ArrayBuffer;
          if (Array.isArray(doc.fileData)) {
            arrayBuffer = new Uint8Array(doc.fileData).buffer;
          } else {
            arrayBuffer = doc.fileData as ArrayBuffer;
          }
          const blob = new Blob([arrayBuffer], { type: 'application/pdf' });
          const tempFile = new File([blob], doc.name, { type: 'application/pdf' });
    ...
    // Use Gemini to find relevant sections
    const relevantSections = await findRelevantSections(documentContents, searchQuery);
    setRelevantSections(relevantSections);
...
const TextSelectionModal = () => {
  if (!showTextSelectionModal || !selectedPdfText) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl mx-4 p-6 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Selected Text Found</h3>
              <p className="text-sm text-gray-500">Would you like to search for similar content?</p>
            </div>
          </div>
          <button
            onClick={() => setShowTextSelectionModal(false)}
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Selected Text ({selectedPdfText.length} characters)
          </label>
        ...
        <div className="flex items-center space-x-3">
          <button
  onClick={() => {
    console.log('Modal: Search button clicked with text:', selectedPdfText);
    searchWithSelectedText(true);
  }}
  className="flex-1 py-3 px-4 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white rounded-lg font-medium text-sm transition-all duration-200 flex items-center justify-center space-x-2"
>
            <Search className="h-4 w-4" />
            <span>Search Similar Content</span>
          </button>
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <ul className="text-xs space-y-1 list-disc list-inside">
                <li>AI will search all your documents for similar content</li>
                <li>You'll get the top 5 most relevant sections</li>
                <li>Select sections to generate comprehensive insights</li>
              </ul>
            </div>
          </div>
        </div>
  ...
  setTimeout(() => {
    if (window.AdobeDC) {
      try {
        ...
        // Add text selection indicator overlay
        setTimeout(() => {
          addTextSelectionIndicator();
        }, 1000);
...
const addTextSelectionIndicator = () => {
  const viewerContainer = document.getElementById('adobe-dc-view');
  if (viewerContainer && !viewerContainer.querySelector('.text-selection-indicator')) {
    ...
    setTimeout(() => {
      if (indicator && viewerContainer.contains(indicator)) {
  ...
  <div className="p-4 border-t border-gray-200 bg-gray-50 flex-shrink-0">
    ...
    >
      {isAnalyzing ? (
        ...
        {/* Text Selection Modal - Add this right before the closing div of the main component */}
        {showTextSelectionModal && selectedPdfText && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-2xl max-w-2xl mx-4 p-6 max-h-[80vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
